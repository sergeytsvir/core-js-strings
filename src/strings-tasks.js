...
function findLongestWord(sentence) {
  ...
  const longWord = arr.reduce((start, current) => {
    return current.length > start.length ? current : start;
  }, '');
  return longWord;
}
...
function encodeToRot13(str) {
  ...
  let res = '';
  for (let i = 0; i < str.length; i += 1) {
    if (alphabet.indexOf(str[i]) === -1) res += str[i];
    else res += encodeStr[alphabet.indexOf(str[i])];
  }
  return res;
}
